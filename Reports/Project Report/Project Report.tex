...
\begin{lstlisting}
    ...
    # Load the query from a JSON file
...
In this program, we first define the URL of the RCSB PDB API endpoint. We then open and load the json file provided from a query dictionary to a JSON string using json.dumps(). Next, we set the headers and data for the HTTP request using the requests.post() function. We pass in the URL, headers, and data as arguments, and get back the response from the server.

\clearpage

\subsubsection{Issues with creating a valid JSON}

I came across an issue when creating a valid JSON to pass into the Searh API. Currently the JSON file is only allowing the search for one value. However there is a vast number of other filters and search options a reasercher can use when browsing the pdb bank. Therfore i decdided to consoludate with my supervisor to determine some useful filter options for the reasercher to use whilst working with the framework. Some of these filters included:
